#!/bin/bash
# run_ppo_7b_ablation_discrete_bins.sh — 奖励函数消融实验 3：7B × 8 卡 PPO 全离散三档阶梯奖励训练 (Ablation Exp 3)
# 特性：启用 code_rlvr_discrete_bins.py ({-1.0, 0.0, +1.0} 全离散三档)，消除浮点方差，构建两级势能阶梯
...
REWARD="${HOME_DIR}/project/rewards/code_rlvr_discrete_bins.py"
CKPT_DIR="${HOME_DIR}/project/checkpoints/d4_ablation_7b_discrete_bins"
...
echo "================================================================="
echo "[Ablation Exp 3] 启动 7B × 8 卡 PPO 全离散三档阶梯 ({-1.0, 0.0, +1.0}) 消融训练"
echo "  - 模型路径: $MODEL"
echo "  - 训练数据: $DATA/train_full.parquet (4,518 题)"
echo "  - 评测数据: $DATA/test.parquet (200 题)"
echo "  - 奖励函数: $REWARD (崩溃/语法错 -1.0, 未全过 0.0, 100%全通 +1.0)"
echo "  - 检查点目录: $CKPT_DIR"
echo "  - 训练步数: 50 步 (与 Exp 1/Exp 2/GRPO 对齐，耗时约 40 分钟)"
echo "================================================================="
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='d4_ablation_7b_discrete_bins' \
