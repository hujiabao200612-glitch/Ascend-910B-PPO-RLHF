#!/bin/bash
# run_ppo_7b_ablation_denser.sh — 奖励函数消融实验 5：7B × 8 卡 PPO DenseR 跨类散度信用与组内独特性消融训练 (Ablation Exp 5)
# 特性：启用 code_rlvr_denser.py，将 DenseR 核心机制（散度信用边界平滑 + 独特性加成 + 崩溃惩戒）与 PPO 细粒度时序 GAE 优势估计融为一体
...
REWARD="${HOME_DIR}/project/rewards/code_rlvr_denser.py"
CKPT_DIR="${HOME_DIR}/project/checkpoints/d4_ablation_7b_denser"
...
echo "================================================================="
echo "[Ablation Exp 5] 启动 7B × 8 卡 PPO DenseR (散度信用 + 组内独特性) 消融训练"
echo "  - 模型路径: $MODEL"
echo "  - 训练数据: $DATA/train_full.parquet (4,518 题)"
echo "  - 评测数据: $DATA/test.parquet (200 题)"
echo "  - 奖励函数: $REWARD"
echo "  - 机制图谱: 崩溃/0通 -1.0 | 临界探索 0.10~0.55 | 全通冗长 +1.0 | 全通紧致独特性 +1.3"
echo "  - 优势估计: PPO GAE (gamma=1.0, lambda=0.95)"
echo "  - 检查点目录: $CKPT_DIR"
echo "  - 训练步数: 50 步 (与其他消融实验完全对齐，预计耗时约 40 分钟)"
echo "================================================================="
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='d4_ablation_7b_denser' \
