#!/bin/bash
# run_ppo_7b_ablation_len_efficiency.sh — 奖励函数消融实验 4：7B × 8 卡 PPO 长度/效率感知双目标训练 (Ablation Exp 4)
# 特性：启用 code_rlvr_len_efficiency.py ({-1.0, +1.0, +1.5})，短代码全通爆发加成，兼具高防御与奥卡姆极简
...
REWARD="${HOME_DIR}/project/rewards/code_rlvr_len_efficiency.py"
CKPT_DIR="${HOME_DIR}/project/checkpoints/d4_ablation_7b_len_efficiency"
...
echo "================================================================="
echo "[Ablation Exp 4] 启动 7B × 8 卡 PPO 长度/效率双目标加成消融训练"
echo "  - 模型路径: $MODEL"
echo "  - 训练数据: $DATA/train_full.parquet (4,518 题)"
echo "  - 评测数据: $DATA/test.parquet (200 题)"
echo "  - 奖励函数: $REWARD (未全通 -1.0, 全通长代码 +1.0, 全通短代码 <=250tok +1.5)"
echo "  - 检查点目录: $CKPT_DIR"
echo "  - 训练步数: 50 步 (耗时约 40 分钟)"
echo "================================================================="
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='d4_ablation_7b_len_efficiency' \
