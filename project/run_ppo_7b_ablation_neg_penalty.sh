#!/bin/bash
# run_ppo_7b_ablation_neg_penalty.sh — 奖励函数消融实验 1：7B × 8 卡 PPO 强负惩罚训练
# 特性：启用 code_rlvr_neg_penalty.py (-1.0 强惩罚)，扩大 Advantage 差距，彻底消除边缘致命错误
...
REWARD="${HOME_DIR}/project/rewards/code_rlvr_neg_penalty.py"
CKPT_DIR="${HOME_DIR}/project/checkpoints/d4_ablation_7b_neg_penalty"
...
echo "================================================================="
echo "[Ablation Exp 1] 启动 7B × 8 卡 PPO 强负惩罚 (-1.0 vs +1.0) 消融训练"
echo "  - 模型路径: $MODEL"
echo "  - 训练数据: $DATA/train_full.parquet (4,518 题)"
echo "  - 评测数据: $DATA/test.parquet (200 题)"
echo "  - 奖励函数: $REWARD (做错硬惩罚 -1.0, 全对满分 +1.0)"
echo "  - 检查点目录: $CKPT_DIR"
echo "  - 训练步数: 50 步 (与 GRPO 消融步数对齐，耗时约 40 分钟)"
echo "================================================================="
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='d4_ablation_7b_neg_penalty' \
