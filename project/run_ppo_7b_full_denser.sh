#!/bin/bash
# run_ppo_7b_full_denser.sh — 7B × 8 卡 PPO-DenseR 1 个完整 Epoch 全量主训练 (71 Steps, 4,518 题)
# 核心架构：融合 DenseR 细粒度跨类散度信用 (0.10~0.55) + 组内独特性/奥卡姆剃刀奖赏 (+1.30) + 崩溃硬惩戒 (-1.0)
# 训练规模：完整跑满 1 个 Epoch（4,518 题 / batch_size=64 ≈ 71 steps），挑战全场宏观大满贯纪录！
...
CKPT_DIR="${HOME_DIR}/project/checkpoints/d4_full_7b_denser"
...
echo "================================================================="
echo "[PPO-DenseR Full 71-Step] 启动 7B × 8 卡 1 个完整 Epoch 全量主训练"
echo "  - 模型路径: $MODEL"
echo "  - 训练数据: $DATA/train_full.parquet (4,518 题, 100% 全覆盖)"
echo "  - 评测数据: $DATA/test.parquet (200 题)"
echo "  - 奖励函数: $REWARD (DenseR 散度信用 + 组内独特性)"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='d4_full_7b_denser' \
